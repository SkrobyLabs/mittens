...
func TestRunVersion_JSONHasVersionAndCommitOnly(t *testing.T) {
	...
	versionValue, ok := got["version"]
	if !ok {
		t.Fatalf("JSON output missing \"version\": %v", got)
	}
	if versionString, ok := versionValue.(string); !ok || versionString != "2.4.6" {
		t.Fatalf("JSON version = %v, want \"2.4.6\"", versionValue)
	}

	commitValue, ok := got["commit"]
	if !ok {
		t.Fatalf("JSON output missing \"commit\": %v", got)
	}
	if commitString, ok := commitValue.(string); !ok || commitString != "commit-abc" {
		t.Fatalf("JSON commit = %v, want \"commit-abc\"", commitValue)
	}
}
