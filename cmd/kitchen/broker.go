...
// WorkerBroker exposes PoolManager methods as REST endpoints for worker agents.
...
func (b *WorkerBroker) handlePollTask(w http.ResponseWriter, r *http.Request) {
	...
	if b.pm.PopRecycleFlag(wid) {
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(pollTaskResp{Recycle: true})
		return
	}
	...
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(pollTaskResp{Task: task})
}
