...
func TestClassifyFailure(t *testing.T) {
	tests := []struct {
		...
		{name: "codex websocket 500", reported: "execute codex (exit 1): failed to connect to websocket: HTTP error: 500 Internal Server Error, url: wss://api.openai.com/v1/responses", want: FailureInfrastructure},
		{name: "bare websocket drop", reported: "websocket closed unexpectedly", want: FailureInfrastructure},
		{name: "502 bad gateway", reported: "provider returned 502 Bad Gateway", want: FailureInfrastructure},
		{name: "503 service unavailable", reported: "upstream 503 Service Unavailable", want: FailureInfrastructure},
		{name: "504 gateway timeout still classified as timeout", reported: "HTTP 504 gateway timeout", want: FailureTimeout},
		{name: "failed to connect", reported: "dial tcp: failed to connect to upstream", want: FailureInfrastructure},
